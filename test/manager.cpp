...
TEST(ManagerTest, parseOpdsDomAddsEntriesAndParsesSearchMetadata)
{
    ...
    EXPECT_TRUE(manager.parseOpdsDom(doc, "http://example.com", /*readOnly=*/false));
...
TEST(ManagerTest, parseOpdsDomHonorsReadOnly)
{
...

    {
      auto lib = kiwix::Library::create();
      TestManager manager(lib);
      EXPECT_TRUE(manager.parseOpdsDom(doc, "http://example.com", /*readOnly=*/true));
...
>>>>>>> f28a8764 (Changing readFile() method, to support both XML and OPDS formats)
TEST(Manager, reload)
{
  auto lib = kiwix::Library::create();
  kiwix::Manager manager(lib);
