                       ...
                       const std::string& libraryPath)
{
  ...
  if (result) {
    this->parseOpdsDom(doc, urlHost, libraryPath, readOnly);
    return true;
  }
  ...
  bool trustLibrary)
{
  ...
  FileFormat format = detectFormat(content);
  if (format == FileFormat::OPDS) {
    retVal = this->readOpds(content, "", readOnly, path);
  } else {
    retVal = this->readXml(content, readOnly, path, trustLibrary);
  }
